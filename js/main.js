...
var update100ms = window.setInterval(function () {
  ...
  if (true) {
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    chrome.storage.sync.get({ keuzeBtn: true, darkMode: true, keuzeMode: "table", zermelo: false }, (items) => {
      /// Keuze page
      if (!document.getElementById("coverDivKeuze")) {
  ...
  /// Chrome storage
  chrome.storage.sync.get({ cijfers: false, hideHelpBtn: true, hidePfp: false, widgetCustomHigh: 385, widgetCustomLow: 0, darkMode: true, hideBestellenBtn: false, customPfp: false, widgetDrag: true, hideZoekenBtn: true, customVandaag: false, maxLaatsteCijfers: 10, showTime: false, oppBtn: true, koppelingenBtn: true, clockSecondBtn: true, sidebarSmallBtn: false, spaceSidebar: false }, (items) => {
    ...
    //~ Set custom pfp
    if (items.customPfp) {
      chrome.storage.local.get({ userImage: "" }, (items) => {
    ...
    // if (items.koppelingenBtn && document.querySelector("body > div.container > div.appbar-host > mg-appbar > div.appbar > div.gripper.ng-scope")) {
    ...
    if (items.customVandaag) {
      ...
      if (vandaagContainer) {
        ...
        if (!document.getElementById("cijfersCard")) {
          ...
          laatsteCijfersText.addEventListener("click", () => {
            currentCijferId = 0;
          });
          ...
          cijferWaarde.addEventListener("click", () => {
            document.getElementById("menu-cijfers").click();
          });
          ...
          cijferOmschrijving.addEventListener("click", () => {
            document.getElementById("menu-cijfers").click();
          });
          ...
          cijferVak.addEventListener("click", () => {
            document.getElementById("menu-cijfers").click();
          });
    ...
    if (items.showTime) {
      ...
      if (timeSpan.textContent !== timeNow) [(timeSpan.textContent = timeNow)];
    }
  ...
  if (currentLocationSplit === "magister.net/magister/#/att-absence") {
    ...
    attAbsDashDOM
      .querySelector(".wrapper > dna-link-card")
      .shadowRoot.querySelectorAll(".arrow")
      .forEach((item) => {
  ...
  if (currentLocationSplit === "magister.net/magister/#/att-absence/absence-overview-student") {
    ...
    *::-webkit-scrollbar-thumb:hover {
    ...
    dnaTabs.querySelectorAll("dna-tab-bar > dna-tab-button").forEach((btn) => {
      btn.style.color = "var(--mooie-text-color)";
    });
    ...
    if (berichtenIframe) {
      if (setBerichtenIframeDown) {
        berichtenIframe.contentWindow.document.addEventListener("keydown", (event) => {
          document.dispatchEvent(new KeyboardEvent("keydown", { key: event.key, code: event.code }));
        });
        setBerichtenIframeDown = false;
      }
      if (setBerichtenIframeUp) {
        berichtenIframe.contentWindow.document.addEventListener("keyup", (event) => {
          document.dispatchEvent(new KeyboardEvent("keyup", { key: event.key, code: event.code }));
        });
...
function toggleSearchBox() {
  ...
  if (searchBox.style.display === "none") {
    chrome.storage.sync.get({ hideZoekenBtn: true }, (items) => {
      if (!items.hideZoekenBtn) {
...
//   for (let i = 0; i < 20; i++) {
//     // 20 betekent max 2000 betichten. veranderen voor meer
...
//     if (j % 25 === 0) {
//       // pas dit aan als je rate limit errors krijgt
//       await delay(2000);
//     }
//   }
